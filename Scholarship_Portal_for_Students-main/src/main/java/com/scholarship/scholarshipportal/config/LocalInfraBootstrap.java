...
/**
 * Starts embedded Kafka (KRaft mode) and embedded Redis during Spring Boot startup.
 *
 * No Docker required. No external Kafka or Redis installation required.
 * Just run: mvn spring-boot:run
 *
 * Kafka topics are pre-created and spring.kafka.bootstrap-servers is set dynamically.
 * Redis starts on port 6379 (or skips if an external Redis is already running).
 */
public final class LocalInfraBootstrap {

    private static final Logger log = LoggerFactory.getLogger(LocalInfraBootstrap.class);

    private static final String[] KAFKA_TOPICS = {
        "application-events", "user-events", "notification-events", "audit-events",
        "application-events.DLT", "notification-events.DLT"
    };
